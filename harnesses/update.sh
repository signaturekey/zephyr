...
verify_claude_installation() {
  installed_skill=$claude_skill_root
  installed_manifest="$installed_skill/references/assets.sha256"
  require_regular_file "$installed_manifest"
  verify_installed_file "$installed_skill/SKILL.md" "$installed_manifest" "harnesses/claude-code/SKILL.md"
  verify_manifest_group "$installed_skill" "$installed_manifest" "roles/" "roles" ".md"
  verify_manifest_group "$installed_skill" "$installed_manifest" "schemas/" "schemas" ".json"
...
if [ "$update_claude" = yes ]; then
  reject_symlink_components "$claude_skill_root"
  reject_symlink_components "$claude_agents_dir"
  if ! surface_is_absent "$claude_skill_root" "$claude_agents_dir" ".md"; then
    verify_claude_installation
    claude_was_installed=yes
  fi
fi
...
rollback_update() {
  ...
  if [ "$mutation_started" = yes ] && [ "$update_complete" != yes ]; then
    ...
    if [ "$update_claude" = yes ]; then
      move_new_surface_aside "$claude_skill_root" "$claude_agents_dir" "$backup_root/failed-new/claude" "$backup_root/previous/claude" "$repo_root/harnesses/claude-code/agents" ".md" "$publication_started"
      restore_old_surface "$claude_skill_root" "$claude_agents_dir" "$backup_root/previous/claude"
