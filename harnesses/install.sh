...
if [ "$install_claude" = yes ]; then
  ...
  check_destination "$repo_root/harnesses/claude-code/SKILL.md" "$claude_skill_root/SKILL.md"
  check_destination "$repo_root/harnesses/assets.sha256" "$claude_skill_root/references/assets.sha256"
  for source_path in "$repo_root"/roles/*.md; do
    check_destination "$source_path" "$claude_skill_root/references/roles/${source_path##*/}"
  done
  for source_path in "$repo_root"/schemas/*.json; do
    check_destination "$source_path" "$claude_skill_root/references/schemas/${source_path##*/}"
  done
  for source_path in "$repo_root"/harnesses/claude-code/agents/zephyr-*.md; do
    require_source "$source_path"
    check_destination "$source_path" "$claude_agents_dir/${source_path##*/}"
    check_destination "$source_path" "$claude_skill_root/references/agents/${source_path##*/}"
  done
fi
...
if [ "$install_claude" = yes ]; then
  install_file "$repo_root/harnesses/claude-code/SKILL.md" "$claude_skill_root/SKILL.md"
  install_file "$repo_root/harnesses/assets.sha256" "$claude_skill_root/references/assets.sha256"
  for source_path in "$repo_root"/roles/*.md; do
    install_file "$source_path" "$claude_skill_root/references/roles/${source_path##*/}"
  done
  for source_path in "$repo_root"/schemas/*.json; do
    install_file "$source_path" "$claude_skill_root/references/schemas/${source_path##*/}"
  done
  for source_path in "$repo_root"/harnesses/claude-code/agents/zephyr-*.md; do
