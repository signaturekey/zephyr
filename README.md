...
read-only MCP-операции skill также читает один уровень прямых Jira-, Confluence- и
Bitbucket-ссылок из корневой Jira issue или PR. Каждый объект сохраняется в приватный
временный Markdown-файл, передаётся через отдельный `--context` и удаляется после
запуска даже при ошибке.

Обход ограничен глубиной 1 и 20 внешними объектами с дедупликацией. Skill не обходит
деревья страниц, не продолжает переходы из уже раскрытых объектов, не читает
комментарии или историю без явного запроса и не использует неоднозначные read/write
MCP tools. Недоступный, неоднозначный, пропущенный по лимиту, устаревший или усечённый
